    ...
    def mock_client(self):
        ...
        mock_papers = {
            "paper1": {
                ...
                "matches": {"title": ["neural", "networks"], "abstract": ["time", "series"]}
            },
            "paper2": {
...
class TestExportPapersTool:
    ...
    @pytest.mark.asyncio
    async def test_export_papers_without_pdfs(self, mock_extract, mock_download, mock_client_class, temp_dir):
        """Test export_papers functionality without PDF download."""
        # Setup mocks
        mock_client = Mock()
        mock_papers = [
            Mock(id="paper1", title="Test Paper", authors=["Alice"], 
                 abstract="Test abstract", venue="ICLR.cc/2024")
        ]
    ...
    @patch('openreview_mcp_server.tools.export_papers.extract_up_to_references')
    @pytest.mark.asyncio
    async def test_export_papers_with_pdfs(self, mock_extract, mock_download, mock_client_class, temp_dir):
        ...
        mock_extract.return_value = "Extracted text content from the paper up to references."
        ...
        assert text_data["extracted_text"] == "Extracted text content from the paper up to references."
