...
async def handle_search_user(arguments: Dict[str, Any]) -> List[types.TextContent]:
    ...
    try:
        ...
        # Search for user
        profile = client.find_user_by_email(
            email, with_publications=include_publications
        )

        if not profile:
            return [
                types.TextContent(
                    type="text", text=f"No user profile found for email: {email}"
                )
            ]
        ...
        if include_publications and profile.publications:
            ...
            if len(profile.publications) > 5:
                result += (
                    f"  ... and {len(profile.publications) - 5} more publications\n"
                )
        ...
        return [
            types.TextContent(type="text", text=f"Error searching for user: {str(e)}")
        ]
