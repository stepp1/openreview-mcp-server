...
async def handle_get_conference_papers(
    ...
    try:
        ...
        if not venue or not year:
            return [
                types.TextContent(
                    type="text", text="Error: Both venue and year are required"
                )
            ]
        ...
        if not papers:
            return [
                types.TextContent(
                    type="text", text=f"No papers found for {venue} {year}"
                )
            ]
        ...
        return [
            types.TextContent(
                type="text", text=f"Error fetching conference papers: {str(e)}"
            )
        ]
