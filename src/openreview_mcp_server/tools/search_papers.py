...
async def handle_search_papers(arguments: Dict[str, Any]) -> List[types.TextContent]:
    ...
    try:
        ...
        for i, paper_result in enumerate(all_results, 1):
            ...
            result += f"   Matched Fields: {', '.join(paper_result['matches'].keys())}\n"
            result += f"   URL: https://openreview.net/forum?id={paper_result['paper_id']}\n"
            
            # Show snippet of abstract if it was searched
            if 'abstract' in paper_result['matches']:
                abstract_snippet = paper_result['abstract'][:150] + "..." if len(paper_result['abstract']) > 150 else paper_result['abstract']
