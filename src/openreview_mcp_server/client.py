...
class OpenReviewClient:
    ...
    def find_user_by_email(
        self, email: str, with_publications: bool = True
    ) -> Optional[Profile]:
        """Find a user profile by email address."""
        try:
            profiles = openreview.tools.get_profiles(
                self.client, [email], as_dict=True, with_publications=with_publications
            )
            ...
            return Profile(
                ...
                emails=emails,
                name=(
                    profile_data.content.get("name", {}).get("value")
                    if isinstance(profile_data.content.get("name"), dict)
                    else profile_data.content.get("name")
                ),
                relations=relations,
    ...
    def get_conference_papers(
        self, venue_id: str, year: Optional[str] = None
    ) -> List[Paper]:
        ...
        try:
            ...
            # Get submissions
            submissions, accepted_submissions = self._get_submissions(
                full_venue_id, api_v1
            )
    ...
    def _get_submissions(self, venue_id: str, api_v1: bool = True):
        ...
        if api_v1:
                ...
                for decision_note in all_decision_notes:
                    if "Accept" in decision_note["content"]["decision"]:
                        accepted_submissions.append(
                            blind_notes[decision_note["forum"]].details["original"]
                        )
            ...
            accepted_submissions = self.client.get_all_notes(
    ...
    def _parse_publication(self, publication) -> Optional[Paper]:
        ...
        try:
            ...
            venue = getattr(
                publication,
                "invitation",
                publication.content.get("venueid", {}).get("value", "Unknown"),
            )
    ...
    def _search_submissions_dict(
        self,
        dict_submissions: Dict,
